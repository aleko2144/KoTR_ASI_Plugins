...
public:
	...
	void Reset(){
		...
		/////////////////
		engine_prefix  = GetPrivateProfileStr(m_car_prefix, "engine_prefix", m_car_prefix, ".\\SEMod_sounds.ini");
		m_accel_rpm = (GetPrivateProfileFloat(m_car_prefix, "accel_rpm", "0", ".\\SEMod_sounds.ini") / 100);
		/////////////////
		use_reverse_signal = GetPrivateProfileIntA(m_car_prefix.c_str(), "reverse_signal", 0, ".\\SEMod_sounds.ini");
		simulate_air_brakes = GetPrivateProfileIntA(m_car_prefix.c_str(), "simulate_air_brakes", 0, ".\\SEMod_sounds.ini");
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL,DWORD fdwReason,LPVOID lpvReserved)
{
	...
	switch(fdwReason)
	{
		...
		{
			//if (GetPrivateProfileIntA("Mod", "Enabled", 0, ".\\SEMod_sounds.ini")){
			if (GetPrivateProfileIntA("COMMON", "CustomSounds", 0, ".\\SEMod.ini")){
				//DisplayConsole();
				AttachHooks();
				SetTimer(0, 0, 50, (TIMERPROC)OnTimer);

				//stereo_enabled = (bool)GetPrivateProfileIntA("Mod", "stereo_enabled", 0, ".\\SEMod_sounds.ini");
				stereo_enabled = (bool)GetPrivateProfileIntA("COMMON", "CustomSounds_stereo", 0, ".\\SEMod.ini");
				reverse_signal_time = GetPrivateProfileIntA("Mod", "reverse_signal_time", 800, ".\\SEMod_sounds.ini");
				air_brakes_time = GetPrivateProfileIntA("Mod", "air_brakes_time", 20000, ".\\SEMod_sounds.ini");
			}
